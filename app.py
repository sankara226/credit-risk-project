...
def load_warehouse_data() -> pd.DataFrame:
    ...
    if not db_file.exists():
        st.error(
            f"Localized system database workspace missing at {db_file}. Execute 'python main.py' to generate artifacts."
        )
        st.stop()
...
st.markdown(
    "Automated diagnostic system analyzing default probabilities across balances, income tiers, and student cohorts."
)
st.markdown("---")
...
with v_col2:
    ...
    sns.histplot(df["income"], kde=True, bins=40, color="#2ecc71", ax=ax3)
    st.pyplot(fig3)
